...
expect.extend({
	[NAME](received: unknown, name: string) {
		...
		// console.log(received.map(({ name = '' }) => name));

		const config =
			received.find(({ name: configName }) => configName?.endsWith(name));

		const pass = config !== undefined;

		const message = (): string =>
			`${matcherHint(NAME, 'received', '', options)}\n\n` +
			`Expected config ${printExpected(`${process.env.npm_package_name}/${name}`)} ${
				pass ? 'not' : ''
			} to be included\n`;
