const { DEBUG = '', NODE_ENV = 'production', PACKAGE_JSON = `${process.cwd()}/package.json` } = process.env;
...
const prodDependency = (name: string) =>
	...
	Object.keys(bundledDependencies).includes(name);

const devDependency = (name: string) =>
...
const usingTypeScript = devDependency('typescript');
const usingReact = prodDependency('react');
const usingJest = devDependency('jest');
const usingCypress = devDependency('cypress');
const usingGraphql = prodDependency('graphql');

const usingTestingLibraryDom = devDependency('@testing-library/jest-dom');
const usingTestingLibraryReact = devDependency('@testing-library/react');
const usingTestingLibraryEvents = devDependency('@testing-library/user-event');

const usingTestingLibrary =
	usingTestingLibraryDom ||
	usingTestingLibraryReact ||
	usingTestingLibraryEvents;
...
if (isDebug) {
	...
	console.info(`${usingTypeScript ? '✓' : '🆇'} TypeScript`);
	console.info(`${usingReact ? '✓' : '🆇'} React`);
	console.info(`${usingJest ? '✓' : '🆇'} Jest`);
	console.info(`${usingTestingLibrary ? '✓' : '🆇'} @testing-library`);
	console.info(`${usingCypress ? '✓' : '🆇'} Cypress`);
	console.info(`${usingGraphql ? '✓' : '🆇'} Graphql`);
...
export const recommended = Object.freeze({
	parserOptions: {
		ecmaVersion: 'latest',
		sourceType: pkg.type === 'module' ? 'module' : 'script',
	},
	env: {
		es6: true,
		commonjs: true,
	...
	overrides: [
		...
		usingJest ? {
			...
			plugins: [
				'jest',
			].concat(...(usingTestingLibrary ? ['testing-library'] : [])),
			extends: [
				'plugin:jest/recommended',
				usingTestingLibraryDom ? 'plugin:testing-library/dom' : null,
				usingTestingLibraryReact ? 'plugin:testing-library/react' : null,
				'plugin:prettier/recommended', /* <-- must always be last */
			].filter(Boolean),
			files: [
				'**/*.{spec,test}.[jt]s?(x)',
