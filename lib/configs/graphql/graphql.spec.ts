...
describe(`plugin:${process.env.npm_package_name}/graphql`, () => {
	...
	beforeAll(() => {
		...
		Object.defineProperties(process.env, {
			PACKAGE_JSON: {
				get: jest.fn()
					.mockReturnValue(`${process.cwd()}/test/resources/fixtures/gql/package.json`)
			},
		});
	});

	beforeEach(async () => {
		({ configs, error, result } = await lintFixtureFile(graphql));
